from scripts.create_and_deploy import deploy_and_create, deploy_and_create2
from scripts.create_metadata import upload_developer_data_to_ipfs
from scripts.defensive_scripts import defensive
...
def test_defensive():
    key = defensive.generate_secret_key(32)
    plaintext = b"This is a top secret message!"
    ciphertext, iv = defensive.encrypt_AES(key, plaintext)
    print(f"Ciphertext: {ciphertext.hex()}")
    print(f"IV: {iv.hex()}")

    decrypted_plaintext = defensive.decrypt_AES(key, ciphertext, iv)
