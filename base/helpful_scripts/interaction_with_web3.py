...
from base.scripts_for_contract.developer_contract.interaction_with_developer_contract import (
    DeveloperContract,
)
from base.scripts_for_contract.buy_game_contract.interaction_with_buy_game_contract import (
    BuyGameContract,
)
from base.scripts_for_contract.customer_contract.interaction_with_customer_contract import (
    CustomerContract,
)
...
def get_w3():
    json_file = get_json("infura_info")
...
def convert_info_json(info, name):
    ...
    w3 = get_w3()
    abi = get_json(
        f"base/scripts_for_contract/{name}_contract/info/{contract_address}"
    )["abi"]
...
def get_web3_info(name):
    # base\scripts_for_contract\customer_contract\info\customer_contract_info.json
    info = get_json(
        f"base/scripts_for_contract/{name}_contract/info/{name}_contract_info"
    )
